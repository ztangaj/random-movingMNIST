...
#     Srivastava et al
# by Tencia Lee, refactored by Tony C Tong
...
def generate_moving_mnist(digits, motions, shape=(64, 64), num_frames=30, num_sequences=1, original_size=28, random_speed=False):
    '''
    ...
    for img_idx in range(num_sequences):
        ...
        for i in range(len(digits)):
            if motions[i] in ["vertical", "horizontal", "zigzag"]:
                ...
                if not random_speed:
                    speed = np.random.randint(5) + 2 # TODO: This part need to be changed
                    veloc = [speed*math.cos(theta), speed*math.sin(theta)]
                elif random_speed:
                    veloc = [math.cos(theta), math.sin(theta)]
                motion_dict[i] = {
                    "veloc": veloc
                }
        ...
        for frame_idx in range(num_frames):
            ...
            for i in range(len(digits)):
                if motions[i] in ["vertical", "horizontal", "zigzag"]:
                    if random_speed:
                        current_speed = np.random.randint(5) + 2
                    elif not random_speed:
                        current_speed = 1
                    for j in range(2):
                        new_pos = positions[i][j] + motion_dict[i]["veloc"][j]*current_speed
                        if new_pos < -2 or new_pos > lims[j] + 2:
                            motion_dict[i]["veloc"][j]*=-1
                        positions[i][j] += motion_dict[i]["veloc"][j]*current_speed
                elif motions[i] in ["circular_clockwise", "circular_anticlockwise"]:
...
def main(digits, motions, dest, frame_size=64, num_frames=30, num_sequences=1, original_size=28, random_speed=False):
    ...
    dat, action_vectors = generate_moving_mnist(shape=(frame_size, frame_size), num_frames=num_frames, num_sequences=num_sequences,
                                digits=digits, motions=motions, original_size=original_size, random_speed=random_speed)
    ...
    for i in range(num_sequences):
        ...
        # Disable this because we don't need
